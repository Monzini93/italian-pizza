(function () {
  const corpoCardapio = document.getElementById('corpo-cardapio');
  ...
  function render() {
    ...
    if (itens.length === 0) {
      corpoCardapio.innerHTML = '<tr><td colspan="5" class="p-6 text-center text-italian-brown-light">Nenhum item no cardápio. Clique em "Novo item" para cadastrar.</td></tr>';
      return;
    }
    corpoCardapio.innerHTML = itens.map(item => `
      <tr class="hover:bg-amber-50 border-b border-amber-100">
        ...
        <td class="p-3 text-italian-brown">${item.categoria || '—'}</td>
    ...
    corpoCardapio.querySelectorAll('.editar-item').forEach(btn => {
      btn.addEventListener('click', function () {
        abrirModalEditar(btn.dataset.id);
      });
    });
    corpoCardapio.querySelectorAll('.apagar-item').forEach(btn => {
      btn.addEventListener('click', function () {
        if (confirm('Remover este item do cardápio?')) {
          removeItemCardapio(btn.dataset.id);
          render();
        }
      });
    });
